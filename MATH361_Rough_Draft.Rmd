...
# Plot 1
plot1 <- ggplot(df, aes(x = day_number, y = total_trips)) +
  geom_point() +
  labs(
    x = "Day Number (n)",
    y = "Total Bike Ridership"
  ) +
  theme_minimal()

# Plot 2
plot2 <- ggplot(df, aes(x = day_number, y = total_durations)) +
  geom_point() +
  labs(
    x = "Day Number (n)",
    y = "Combined Ride Durations (secs)"
  ) +
  theme_minimal()

# Combine plots side-by-side
combined_plot <- (plot1 + plot2) + plot_annotation(title = "Bike Ridership in Seattle beginning 13 October 2014")
...
cor(df$Precipitation_In,df$total_trips)
...
#powerTransform(cbind(df$total_trips,df$Mean_Tempe#rature_F,df$Mean_Humidity,df$MeanDew_Point_F,comb#ined$Precipitation_In,combined$Mean_Wind_Speed_MPH,combined$M#ean_Visibility_Miles))

#This fails as powerTransform needs arguments to be strictly positive and the min
#of Precipitation and Wind speed are 0

#powerTransform(cbind(combined$total_trips,combined$Mean_Tempe#rature_F,combined$Mean_Humidity,combined$MeanDew_Point_F,comb#ined$Mean_Visibility_Miles))
