...
\item \texttt{weekday\_weekend} was created based on \texttt{date}, according to whether the day was a weekday or Saturday or Sunday.
...
The equation for our final regression model is:
  ...
  \texttt{total\_trips} = 485.34 – 6.20(\texttt{Mean\_Humidity}) + 8.10(\texttt{MeanDew\_Point}) – 103.49(\texttt{Precipitation\_In}) – 8.85(\texttt{Mean\_Wind\_Speed}) - 34.11(\texttt{fall\_winter}) - 50.23(\texttt{Weekend}) 
...
Our work here suggests a path forward for bike share systems looking to bolster their operations and planning with weather data. However, Seattle is a city with temperate weather/climate. These results are not readily generalizable to all cities because when its too hot, people will also not ride bikes!
