...
compact_correlations <- data.frame(
  ...
  Min = c(
    round(cor(df$Min_TemperatureF, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Min_Visibility_Miles, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Min_Dewpoint_F, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Min_Humidity, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Min_Sea_Level_Pressure_In, df$total_trips, use = 'na.or.complete'), 4)
  ),
  Mean = c(
    round(cor(df$Mean_Temperature_F, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Mean_Visibility_Miles, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$MeanDew_Point_F, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Mean_Humidity, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Mean_Sea_Level_Pressure_In, df$total_trips, use = 'na.or.complete'), 4)
  ),
  Max = c(
    round(cor(df$Max_Temperature_F, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Max_Visibility_Miles, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Max_Dew_Point_F, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Max_Humidity, df$total_trips, use = 'na.or.complete'), 4),
    round(cor(df$Max_Sea_Level_Pressure_In, df$total_trips, use = 'na.or.complete'), 4)
  )
)
...
plot(df$Max_Wind_Speed_MPH, df$total_trips, 
     ...
     ylab="Total Rides", 
     sub=paste("cor(x,y) =", round(cor(df$Max_Wind_Speed_MPH, df$total_trips, use='na.or.complete'), 4)))
# Middle plot 
par(mar=c(5, 2.25, 4, 2.25)) # Reduce left margin
plot(df$Mean_Wind_Speed_MPH, df$total_trips, 
     xlab="Mean Wind Speed (MPH)", 
     ylab="", 
     sub=paste("cor(x,y) =", round(cor(df$Mean_Wind_Speed_MPH, df$total_trips, use='na.or.complete'), 4)))
# Right-most plot
par(mar=c(5, 0.5, 4, 4)) # Reduce left margin
plot(df$Max_Gust_Speed_MPH, df$total_trips, 
     xlab="Max Gust Speed (MPH)", 
     ylab="", 
     sub=paste("cor(x,y) =", round(cor(df$Max_Gust_Speed_MPH, df$total_trips, use='na.or.complete'), 4)))
```
## Outliers
